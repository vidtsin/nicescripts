...
class niceDevices():
	...
	def createData(self, model, data):
		...
		return self.serverAcess.execute(self.dbName, self.loginId, self.password, model, "create", data)

	def callFunction(self, model, data, id):

		self.serverAcess.execute(self.dbName, self.loginId, self.password, model, data, id)